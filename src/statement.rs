...
macro_rules! impl_statement_query {
    ($A:ty) => {
        #[inline]
        fn query(&self) -> cdbc::query::Query<'_, Self::Database, $A> {
            cdbc::query::query_statement(self)
        }

        #[inline]
        fn query_with<'s, A>(&'s self, arguments: A) -> cdbc::query::Query<'s, Self::Database, A>
        where
        ...
        fn query_as<O>(
            ...
            Self::Database,
            O,
            <Self::Database as cdbc::database::HasArguments<'_>>::Arguments,
        >
        where
        ...
        fn query_as_with<'s, O, A>(
            &'s self,
            arguments: A,
        ) -> cdbc::query_as::QueryAs<'s, Self::Database, O, A>
        where
            O: for<'r> cdbc::from_row::FromRow<
                'r,
                <Self::Database as cdbc::database::Database>::Row,
        ...
        fn query_scalar_with<'s, O, A>(
            &'s self,
            arguments: A,
        ) -> cdbc::query_scalar::QueryScalar<'s, Self::Database, O, A>
        where
            (O,): for<'r> cdbc::from_row::FromRow<
                'r,
                <Self::Database as cdbc::database::Database>::Row,
