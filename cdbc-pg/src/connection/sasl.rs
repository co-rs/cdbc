...
pub(crate) fn authenticate(
    ...
    let cont = match stream.recv_expect(MessageFormat::Authentication)? {
    ...
    stream.send(SaslResponse(&client_final_message))?;

    let data = match stream.recv_expect(MessageFormat::Authentication)? {
