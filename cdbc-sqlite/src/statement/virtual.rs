...
pub struct VirtualStatement {
    ...
    pub handles: SmallVec<[StatementHandle; 1]>,

    // each set of columns
    pub columns: SmallVec<[Arc<Vec<SqliteColumn>>; 1]>,

    // each set of column names
    pub column_names: SmallVec<[Arc<HashMap<UStr, usize>>; 1]>,
}

pub struct PreparedStatement<'a> {
    pub handle: &'a mut StatementHandle,
    pub columns: &'a Arc<Vec<SqliteColumn>>,
    pub column_names: &'a Arc<HashMap<UStr, usize>>,
}

impl VirtualStatement {
    pub fn new(mut query: &str, persistent: bool) -> Result<Self, Error> {
        query = query.trim();
