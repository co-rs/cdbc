...
use crate::statement::{StatementHandle, StatementWorker};
use crate::{SqliteColumn, SqliteError, SqliteRow, SqliteValue};
use cdbc::HashMap;
use bytes::{Buf, Bytes};
use libsqlite3_sys::{
...
impl VirtualStatement {
    ...
    pub(crate) fn reset(&mut self, worker: &mut StatementWorker) -> Result<(), Error> {
        self.index = 0;
